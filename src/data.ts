...
export const VOCABULARY_DATA = {
  ...
  principles: {
    envelopeIsTheRhythm:
      "Every channel (touch, light, tone) renders the identical envelope. What the hand feels, the eye sees and the ear hears.",
    severityIsIntensity:
      "Severity never changes a rhythm. Where the device has validated amplitude control, touch grades the level in four distinct strengths so DeafBlind users read it by hand; marks and color count the step for the eye; light and sound deliver at full power from DANGER COMING upward, because they exist to reach. A device without amplitude control renders the rhythm unchanged and lets marks, color and words carry the level.",
    unknownPresumedDangerous:
      "A warning whose severity cannot be read is presumed dangerous: Unknown is stated as unknown in language and presented at Severe-equivalent intensity, never gentle.",
    allClearIsAffirmativeRelease:
      "The all-clear is an affirmative, authenticated message naming what ended, presented calmly with the gentle self-terminating release cue. The absence of an alert is never evidence of safety: silence can equally mean a dead battery, no coverage, or a failed device.",
    mimetic:
      "WATER, GROUND, STORM and FIRE are drawn from the hazard's own temporal signature, converging with the iconicity of sign languages; THREAT, TEST and OTHER are deliberately abstract. Mimesis is a design hypothesis under human testing. Future patterns MUST differ from every existing pattern on at least two axes and SHOULD be mimetic where the hazard offers a signature.",
    photosensitivity:
      "Light renderers never exceed 3 flashes per second or 6 state transitions per second (bounds drawn from WCAG 2.3.1, whose full thresholds also cover area, luminance and red flash), and MUST offer a steady-light path for users who identify as photosensitive.",
    repetition:
      "Urgent alerts repeat the full pattern back to back for a bounded, human-factors-validated interval or until acknowledged, whichever comes first, subject to alert-originator suppression instructions, the user's accessibility settings and device safety limits; the alert then persists as an accessible status with periodic reminders at validated intervals. Presentation is never indefinite. The all-clear release is the inverse carrier: it self-terminates on its own.",
    fallback:
      "A device that cannot render a pattern falls back to its platform's common cadence, never to silence, except where the alert originator has validly suppressed that modality.",
    loopSeamlessness:
  ...
  families: {
    GROUND: {
      ...
      mimesis:
        "The ground does not pause: one long unbroken hold, like the sustained-shaking signs for earthquake. Looped, it fuses into one continuous shake. Nothing else is unbroken.",
      edge: "hard",
      pulses: [[4000, 0]],
    },
    WATER: {
      ...
      mimesis:
        "Waves keep coming: long soft rolls, a breath apart, like the rolling hand motion of the signs for waves. The cadence is steady under looping.",
      edge: "soft",
      pulses: [
        [2000, 1000],
        [2000, 1000],
    ...
    STORM: {
      ...
      mimesis:
        "Wind arrives in gusts that slam: a sharp knock every one and a half seconds, steady under looping.",
      edge: "hard",
      pulses: [
        [500, 1000],
        [500, 1000],
    ...
    FIRE: {
      ...
      mimesis:
        "Flames flicker fast: ten rapid pulses, like the fingers of the sign for fire flickering upward. Nothing else is this fast. 2.5 flashes per second, inside the 3 per second bound, and the flicker stays exactly 2.5 per second across the loop boundary.",
      edge: "hard",
      pulses: [
        [200, 200],
        [200, 200],
    ...
    THREAT: {
      ...
      mimesis:
        "A human threat announces itself: one hard knock at the door, one long push, five rapid fires, then a full stop. The stop is long enough that the next knock lands out of real silence. A sequence nothing natural makes.",
      edge: "soft",
      pulseEdgeOverrides: {
        "0": "hard",
      },
    ...
    TEST: {
      ...
      mimesis:
        "A quick gentle double tap, a long quiet, then the double tap again: checking, not warning. A drill must never feel like an emergency.",
      edge: "soft",
      fixedLevel: 0.3,
      pulses: [
        [200, 200],
    ...
    OTHER: {
      meaning:
        "any hazard outside the named families (health, chemical, infrastructure)",
      mimesis:
        "Three steady calls, evenly spaced, then a clean breath: attention without imitation, reserved for what has no shape of its own.",
      edge: "hard",
      pulses: [
        [1000, 2000],
        [1000, 2000],
    ...
    ALL_CLEAR: {
      ...
      mimesis:
        "The release: one long soft press easing away, a real breath, a short settling press. Like a breath out, easing pressure meaning done (protactile-native). A hypothesis under human testing, like all mimesis claims.",
      pulses: [
        [1200, 800],
        [400, 0],
      ],
      ...
      character: {
        principle:
          "Identity lives in the timing alone and must be decodable on a pitchless piezo. Spectral and intensity character is channel-idiomatic, encodes valence (falling means over), never identity, and may never add onsets.",
        touch:
          "Intensity falls inside each press, 25 to 15 percent (two steps where no ramp; floor 15). On-body replay: if the device detects wrist-don, wake, or unlock within 10 minutes of an undelivered release, replay once.",
        light:
          "A ramped swell (about 300 ms ease-in, 400 ms ease-out), never a square flash. AFTERGLOW: after the cue, light holds steady calm green at the same gentle level for 10 minutes, then fades over a minute; a held state is not repetition. Phone: the calm wash holds until dismissed. Building: the beacons that strobed hold steady green, and room lighting returns to full.",
        sound:
          "A falling chime, not the alert voice: the long press glides about 880 to 660 Hz, the settle rests at 440 Hz, intensity decaying inside each unit; never the flat alert carrier. Phones: the cue once, then speech (for example FLOOD WARNING ENDED) starting about 1.5 s after, never overlapping. Voice-capable building systems: cue then spoken all-clear, at most three times at 60 s spacing. Tone-only outdoor sirens MUST NOT sound the release at all: a siren speaking means danger; route the all-clear to other channels.",
      },
    },
  },
  severity: {
  ...
  rules: {
    longUnitExclusive:
      "No pattern other than GROUND and ALL_CLEAR may contain a unit of 1000 ms or longer, and no future pattern may gain one: the release's long-then-short skeleton stays truncation-proof.",
  },
} as const;
...
export const WEA_ASL_DATA = {
  ...
  sources: {
    templateTexts:
      "FCC DA 25-12, Appendix C (adopted English WEA template messages; United States government work). https://docs.fcc.gov/public/attachments/DA-25-12A1.pdf",
    aslVideos:
      "The FCC's official American Sign Language WEA template videos, referenced by 47 CFR 10.480(f). Canonical page: https://www.fcc.gov/WirelessEmergencyAlert-Templates-ASL (verified via the Internet Archive snapshot of 2026-01-19; fcc.gov rejects non-US automated fetches). Videos are CDI-signed and published on the FCC's YouTube channel.",
    reuse:
      "The FCC's July 2025 originator guidance instructs alert originators to embed or link ASL video for alerts. Template texts and videos are U.S. government works; this file only maps them and adds Three Senses hazard-family categorization.",
  },
  honesty: {
    producedBy:
      "The videos and texts are the FCC's, not IDE's. A template video explains what an alert TYPE means in ASL; it is never a translation of a specific message (agency, location, and time reach ASL users as text, 47 CFR 10.500(e)(3)).",
    mismatch:
      "The adopted text set and the published ASL video set are NOT identical. Two adopted templates have no ASL video (Tornado Emergency, Flash Flood Emergency: the escalation variants), and two published videos have no adopted template text (Evacuation Immediate, Shelter in Place Warning). Recorded as found on 2026-08-25.",
    deferredNoAsl: [
      "All-clear",
      "AMBER alert",
      "Extreme heat",
    ...
    deferredNote:
      "Message types the FCC expressly deferred with no template and no ASL (DA 25-12 para 6 n.32; generic evacuation and shelter-in-place template TEXTS were declined as too vague in para 11, yet their ASL videos exist).",
  },
  families: {
    ...
    capCategoryNote:
      "capCategory is the conventional CAP category for the event (Met, Geo, Health, ...), used as the compose default when the originator sets none.",
  },
  templates: [
    {
      ...
      asl: null,
      aslNote:
        "No ASL video published for the Emergency escalation; the Tornado Warning video is the nearest published asset.",
    },
    {
      id: "tornado-warning",
    ...
    },
    {
      ...
      capCategory: "Met",
      englishText:
        "[SENDING AGENCY]: A FLASH FLOOD EMERGENCY is in effect for [LOCATION] until [TIME]. This is an extremely dangerous and life-threatening situation. Do not attempt to travel unless you are fleeing an area that may flood or are under an evacuation order. [URL]",
      asl: null,
      aslNote:
        "No ASL video published for the Emergency escalation; the Flash Flood Warning video is the nearest published asset.",
    },
    {
      ...
      capCategory: "Met",
      englishText:
        "[SENDING AGENCY]: A FLASH FLOOD WARNING is in effect for [LOCATION] until [TIME]. This is a dangerous and life-threatening situation. Do not attempt to travel unless you are fleeing an area that may flood or are under an evacuation order. [URL]",
      asl: {
    ...
    },
    {
      ...
      capCategory: "Met",
      englishText:
        "[SENDING AGENCY]: A SEVERE THUNDERSTORM WARNING is in effect for [LOCATION] until [TIME] for DESTRUCTIVE 80 mile per hour winds. Take shelter inside a sturdy building, away from windows. Flying objects may be deadly to those outside a sturdy shelter. [URL]",
      asl: {
    ...
    },
    {
      ...
      capCategory: "Met",
      englishText:
        "[SENDING AGENCY]: A SNOW SQUALL WARNING is in effect for [LOCATION] until [TIME]. Slow down or delay travel! Be ready for a sudden drop to near zero visibility and icy roads in heavy snow. [URL]",
      asl: {
    ...
    },
    {
      ...
      capCategory: "Geo",
      englishText:
        "[SENDING AGENCY]: A TSUNAMI WARNING is in effect for [LOCATION] until [TIME]. A series of powerful waves and strong currents may affect coasts near you. You are in danger. Get away from coastal waters. Move to high ground or inland now. Keep away from the coast until local officials say it is safe to return. [URL]",
      asl: {
    ...
    },
    {
      ...
      capCategory: "Geo",
      englishText:
        "EARTHQUAKE DETECTED! Drop, Cover, Hold On. Protect Yourself. [SENDING AGENCY] [URL]",
      asl: {
    ...
    },
    {
      ...
      capCategory: "Infra",
      englishText:
        "[SENDING AGENCY]: A 9-1-1 OUTAGE ALERT is in effect for [LOCATION]. Please contact police, fire, medical, or other emergency services directly at their local phone numbers in case of emergency. If you dial 9-1-1, you may not get help. [URL]",
      asl: {
    ...
    },
    {
      ...
      capCategory: "Geo",
      englishText:
        "[SENDING AGENCY]: An AVALANCHE WARNING is in effect in [LOCATION] until [TIME]. Unstable, fast-moving snow can happen quickly, causing injury or death and can block roads and damage property in affected areas. LEAVE areas near [LOCATION]. DO NOT return to area after evacuation until directed by local officials. Travel in the area is not recommended. Avalanches may run long distances. [URL]",
      asl: {
    ...
    },
    {
      ...
      capCategory: "Safety",
      familyNote:
        "An action message: the family follows the causing hazard chosen by the originator.",
      englishText: null,
      textNote:
        "No adopted template text (declined as too vague, DA 25-12 para 11); the ASL video exists.",
      asl: {

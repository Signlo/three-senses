...
export const STANDARD_VERSION: string = VOCABULARY_DATA.version;
...
const FAMILY_NAMES = Object.keys(VOCABULARY_DATA.families) as FamilyName[];

/** Every family name in the vocabulary, ALL_CLEAR included. */
export function families(): FamilyName[] {
  return [...FAMILY_NAMES];
}

/** The family's normative definition. Throws on an unknown name (R7). */
export function family(name: FamilyName): Family {
  const f = (VOCABULARY_DATA.families as Record<string, unknown>)[name];
  if (!f) throw new Error(`Unknown family "${name}" (R7: no dialects)`);
  return { name, ...(f as Omit<Family, "name">) };
}
 ...
 */
export function timeline(name: FamilyName): Timeline {
  const f = family(name);
...
export function cycleMs(name: FamilyName): number {
  return timeline(name).totalMs;
}

/** When cycle n (0-based) starts, for a loop anchored at t0 (R8). */
export function cycleStart(t0: number, n: number, name: FamilyName): number {
 ...
 */
export function severityLevel(severity: SeverityName | number): number {
  if (typeof severity === "number") return Math.min(1, Math.max(0, severity));
  const level = (SEVERITY_TOUCH_LEVELS as Record<string, number>)[severity];
  if (level === undefined) throw new Error(`Unknown severity "${severity}"`);
  return level;
}
 ...
 */
export function reachLevel(severity: SeverityName | number): number {
  if (typeof severity === "number") return Math.min(1, Math.max(0, severity));
  const level = (SEVERITY_REACH_LEVELS as Record<string, number>)[severity];
  if (level === undefined) throw new Error(`Unknown severity "${severity}"`);
  return level;
}

/** The color-independent mark count for a severity (0 to 3). */
export function severityMarks(severity: SeverityName): number {
  const marks = (SEVERITY_MARKS as Record<string, number>)[severity];
  if (marks === undefined) throw new Error(`Unknown severity "${severity}"`);
  return marks;
}
 ...
 */
export function vibratePattern(name: FamilyName): number[] {
  const f = family(name);
  const out: number[] = [];
  f.pulses.forEach(([on, gap], i) => {
    out.push(on);
    if (i < f.pulses.length - 1) out.push(gap);
  });
  return out;
}

/** Max flashes (on-events) in any sliding 1000 ms window of one cycle. */
export function flashesPerSecond(t: Timeline): number {
...
export function transitionsPerSecond(t: Timeline): number {
...
export function lightIsSafe(t: Timeline): boolean {
  return (
    flashesPerSecond(t) <= LIGHT_BOUNDS.maxFlashesPerSecond &&
...
export function renderLog(name: FamilyName): {
  family: FamilyName;
  events: TimelineStep[];
  cycleMs: number;
} {
 ...
 */
export function conformance(): { pass: boolean; results: ConformanceResult[] } {
  const results: ConformanceResult[] = [];
  const vectors = VECTORS_DATA.vectors as Record<
    string,
    { steps: readonly { at: number; event: string }[]; totalMs: number }
  >;
  ...
  results.push({
    ...
    pass:
      timeline("ALL_CLEAR").steps.length > 0 &&

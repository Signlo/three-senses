...
export function weaTemplateFor(sameCodeOrEvent) {
    ...
    if (byCode.length > 0)
        return byCode.find((t) => t.asl) ?? byCode[0];
    const byName = TEMPLATES.filter((t) => t.name.toUpperCase() === q);
    if (byName.length > 0)
        return byName.find((t) => t.asl) ?? byName[0];
    return null;
}
/**
...
export function familyForEvent(sameCodeOrEvent) {
    ...
    if (/earthquake|landslide|avalanche|volcan/.test(q))
        return "GROUND";
    if (/flood|tsunami|storm surge|dam /.test(q))
        return "WATER";
    if (/tornado|hurricane|cyclone|storm|wind|blizzard|squall/.test(q))
        return "STORM";
    if (/fire/.test(q))
        return "FIRE";
    if (/civil|law enforcement|amber|missing|chemical|radiolog|hazardous|threat|attack/.test(q))
        return "THREAT";
    if (/test|drill|exercise/.test(q))
        return "TEST";
    return "OTHER";
}
function esc(s) {
...
export function composeCap(input) {
    ...
    const template = input.attachAslTemplate === false ? null : weaTemplateFor(input.sameCode ?? input.event);
    ...
    if (input.wea) {
        ...
        if (input.wea.longEn)
            x += parameter("CMAMlongtext", input.wea.longEn);
        if (input.wea.shortEs)
            x += parameter("CMAMtext-Spanish", input.wea.shortEs);
        if (input.wea.longEs)
            x += parameter("CMAMlongtext-Spanish", input.wea.longEs);
    }
